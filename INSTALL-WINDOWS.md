...
```powershell
& { $script = gh api repos/lewenbraun/iban/contents/install.ps1 -H 'Accept: application/vnd.github.raw+json'; if ($LASTEXITCODE -ne 0) { throw 'Cannot download installer' }; & ([scriptblock]::Create(($script -join "`n"))) }
```

This executes the installer maintained in this repository's default branch. It downloads the latest stable release, verifies SHA256 before replacing the executable, retains the previous executable in the printed recovery directory, and restarts the tray. Installation path: `%LOCALAPPDATA%\Iban`. An explicit version can be supplied as `-Tag v0.0.3` when invoking the script block.
...
PowerShell ExecutionPolicy is not needed for `iban.exe`. If Scoop requires RemoteSigned, set it only for your user; do not change it for each update.
...
Pushing a `v*` tag builds and publishes the release, then updates `iban.json` on the default branch using authenticated GitHub CLI downloads. To repair a manifest for an already published release without creating another version:

```powershell
gh workflow run release.yml --repo lewenbraun/iban -f tag=v0.0.3
```
